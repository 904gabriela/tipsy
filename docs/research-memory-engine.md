Base directory for this skill: C:\Users\gabri\AppData\Local\Temp\claude\bundled-skills\2.1.268\bf1333aa2f5a5f4026e590dcc4fc7523\claude-api
...
prompt caching: cache_control, breakpoints, minimum cacheable tokens, TTL, cache invalidation ordering rules, pricing multipliers

---

The final research agent returned after I'd delivered the report. Its findings are substantial and change several recommendations. Addendum:

---

# Addendum — Late Findings That Change the Design

Four of these would have altered what I wrote. Flagged ⚠️.

## A1. ⚠️ Naive persistence can be *worse than no memory*

**TEPA: Revoking Stale Memories for Conflict-Robust Language Agents** ([arXiv 2608.07429](https://arxiv.org/abs/2608.07429)). Under full-reversal drift — a fact that was true becomes false — measured scores:

| strategy | score |
|---|---|
| append-only memory | 0.210 |
| last-write-wins | 0.210 |
| **no memory at all** | **0.309** |
| TEPA (explicit revocation) | 0.950 |

Both naive strategies lose to having no memory. The mechanism: memories are **keyed precedents with validity as an explicit state field**; contradicting evidence under the same key **revokes** the prior (retained for audit, removed from retrieval).

**Change to §5:** `facts` entries need `valid_from` / `valid_until` / `superseded_by`, not just `text` + `known_by`. A slow burn is *made of* reversals — "Ochako thinks Izuku is avoiding her" becomes false, and if the old fact stays retrievable it will keep surfacing. Pair with [Zep/Graphiti](https://arxiv.org/abs/2501.13956)'s **bi-temporal edges**: every fact carries *valid time* (when it was true in-story) and *transaction time* (when you learned it), so you can answer both "what was true on day 40" and "what did Ochako believe on day 40" — which is precisely the §3 knowledge ledger's hardest query.

## A2. ⚠️ Never let the model write an absolute number

**BetterSimTracker** ([ghostd93/BetterSimTracker](https://github.com/ghostd93/BetterSimTracker)) is delta-native by design. Its prompt: *"Do NOT rewrite absolute values; provide per-stat deltas… It is valid to return 0 or negative deltas."* Output is `{name, confidence: 0.0-1.0, delta: {...}, mood, lastThought}`, applied as:

```
clampedDelta = clamp(delta, ±maxDeltaPerTurn)                         # default 15
scale        = (1 - confidenceDampening) + confidence * confidenceDampening   # dampening 0.65
scaledDelta  = round(clampedDelta * scale)
stat         = clamp(stat + scaledDelta, 0, 100)
mood         = confidence < moodStickiness (0.6) ? previousMood : newMood
```

**Change to §8:** my `|Δscore| ≤ 3` clamp was right in spirit but crude. Adopt the full formula — confidence-scaled, clamped, with *stickiness* on categorical fields so a low-confidence read doesn't flip a mood or a stage. A single hallucinated update becomes bounded by construction. Also: BST **seeds first-run characters from defaults with no model call**, explicitly to avoid "phantom delta spikes" — do the same when a background MHA character first speaks.

## A3. ⚠️ Per-field regeneration poisons itself

[Zaakh/SillyTavern-zTracker](https://github.com/Zaakh/SillyTavern-zTracker) spec 24 documents the failure exactly: *"regenerating only one target still leaves other wrong targets in the prompt context; those remaining wrong values can anchor later partial regenerations."* Their fix is a **clear-then-recreate-selected** atomic flow.

**Change to §5:** my "regenerate just this field" UI affordance needs the wrong values *removed from context*, not merely overwritten afterward. zTracker also replaces the bespoke field-def with **JSON Schema draft-07** plus two custom annotations worth copying: `x-ztracker-dependsOn: ["charactersPresent"]` (generation ordering) and `x-ztracker-idKey: "name"` (array-item identity for targeted regeneration).

## A4. ⚠️ Cache-aware injection threshold freezing

Qvink Memory documents the problem I raised in §7 and solves it: *"each new chat message pushes an older message past the injection threshold, potentially changing the injected content and invalidating your prompt cache… you can opt to freeze the injection threshold at its current location, only updating when certain conditions are met."* Triggers (any one): N new messages, N new summaries, or last prompt reached X% of max context.

**Change to §7:** my layout put the volatile block after the history, which is correct — but the *episode/summary* content inside it will churn every turn as the archive boundary advances. Freeze the recall set and only recompute it on those three triggers. Talemate has the only other cache-aware implementation found (`volatile_context_placement()` moves memory recall before or after history depending on caching mode).

## A5. Numbers that strengthen §5's patch recommendation — with a caveat

**PatchBoard** ([arXiv 2605.29313](https://arxiv.org/abs/2605.29313)) replaces inter-agent natural language with **validated JSON Patch mutations over shared structured state**, gated by a deterministic kernel checking (a) schema constraints, (b) role-specific write contracts, (c) runtime invariants — *before* committing transactionally. 630 matched ALFWorld episodes: **84.6% success at 45.5k tokens/task** vs LangGraph's 30.8% at 368.3k. That is ≈2.7× success at ≈8× fewer tokens, and it is essentially the architecture I proposed.

But two counterweights:

- [arXiv 2607.21674](https://arxiv.org/abs/2607.21674) (4,013 runs, 3 models × 3 formats): format choice **reorders model rankings** — JSON Patch 94% on one model, full-file best on another. No universal winner.
- [arXiv 2609.05779](https://arxiv.org/abs/2609.05779): whole-file beat iterative search/replace on *every* metric including LLM-judged code quality. The isolated predictor is **task locality** — diffs win only where mean edit-step count is lowest.
- aider's finding is the sharpest warning: *"Complex edit formats often lead GPT to write worse code **and** make it less successful at formatting the edits correctly."* The format tax hits content quality, not just parse rate. Function-call variants performed *worse* than plain text.

**Net:** my patch-plus-periodic-audit recommendation survives, and the routing rule gets sharper — **patch when the turn touches 1–2 fields; rewrite the bucket when the turn restructures the scene** (arrival, departure, time skip, revelation). That's task locality applied to narrative.

**Open gap worth stating:** no published work benchmarks RFC 6902 on *narrative or character* state. PatchBoard (agent task state), DBPlanBench (query plans), EcoScratch (Scratch programs) are the nearest analogues; Talemate and BetterSimTracker are the closest shipped implementations and neither has published numbers. You will be somewhat on your own here.

## A6. Direct support for the §3 recommendation

**CHARM** ([arXiv 2609.01352](https://arxiv.org/abs/2609.01352)) separates **Boundary-Awareness** from **Boundary-Compliance** and finds hallucination is overwhelmingly a *compliance* failure — models correctly recognise a query is outside the character's knowledge **and answer anyway**. That is the strongest available evidence for my core claim: telling the model what a character doesn't know does not work, because the model already knows and does it anyway. Enforcement (validator + regeneration), not statement.

Two more converging on the same architecture:
- **Belief-State Engine** ([arXiv 2609.10036](https://arxiv.org/abs/2609.10036)) proves LLM+BSE is a sound Markov policy *provided the raw action-observation log is withheld* — "show only the posterior, never the log."
- **Agent-BRACE** ([arXiv 2605.11436](https://arxiv.org/abs/2605.11436)): belief state as a set of atomic NL claims each tagged with an **ordinal certainty label** (`certain`…`unknown`), policy conditions only on the belief. Context stays near-constant in episode length. Add a certainty tag to each `facts[].known_by` entry — "Ochako suspects" versus "Ochako knows" is a distinction your story needs anyway.

Also relevant: **Grounded Continuation** ([arXiv 2605.14175](https://arxiv.org/abs/2605.14175)) — an LLM classifies each utterance into one of 8 epistemic operations, then a *symbolic* engine applies them to a dependency map. Groundedness becomes a graph walk with **zero LLM calls**; prompt ~0.8k tokens where full context is 114k; MemoryAgentBench single-hop 0.46–0.95 → 0.93–0.98. If the §3 validator proves too expensive, this is the cheaper shape: classify the knowledge *operations* in each reply, check them in code.

## A7. Exact Talemate snapshot config (completes §5.1)

I reported the merge semantics; here are the numbers I missed. `AgentAction("update_world_state")`:

| key | default | meaning |
|---|---|---|
| `turns` | **10** | refresh every N *character turns* |
| `focus_lines` | **3** | how many recent messages count as "the current moment" |
| `eviction_threshold` | **3** | drop an entity untouched for 3 passes |
| `max_items` | **10** | bucket cap, stalest dropped first |
| `durable_snapshot` | **True** | delta-merge vs wholesale rebuild |
| `inject_as_scene_memory` | **False** | pin the snapshot into conversation prompts |

Plus **single-flight**: if a snapshot is in flight, the counter stays at its due value and retries next turn rather than overlapping — *because overlapping would double-count eviction*. That's a real bug you'd otherwise ship.

The `CURRENT STATE` block shown to the model **excludes `mentions` and `misses`** so the model re-derives mentions against the current window and never sees the eviction counter. And the injection framing is worth copying verbatim: *"Supplementary, evolving notes… they enrich the context but are not directives: do not recite them, and let the established narrative and the most recent messages take precedence where anything conflicts."*

Two other Talemate cadences: **conditional context pins** re-check every 2 turns (pins with `decay` are skipped until `decay_due == 1` — cheap), and **character progression** runs every 15 rounds with `max_changes: 1, as_suggestions: True`, emitting *function calls* (`add_attribute`, `update_attribute`, `remove_attribute`, `update_description`) that land in `WorldState.suggestions` for manual accept. That is your "propose lore, ask before saving" pipeline, already built, capped and gated.

## A8. Two structures worth considering for §5's schema

- **PHASE-Tree** ([arXiv 2608.06975](https://arxiv.org/abs/2608.06975)) — multi-timescale character state: immutable *identity* root plus mutable *persona / session / moment* layers, each field individually addressable. A within-scene update touches only `moment`; cross-episode drift touches `persona`; unchanged traits are **structurally protected** from being overwritten. This is a cleaner answer than my flat character sheet to "how do I stop mood churn from eroding personality." Ships LongEvoRoleBench, which tests speaking *from* evolved state rather than persona recall.
- **A-MEM** ([arXiv 2502.12110](https://arxiv.org/abs/2502.12110)) — on insert, generate contextual description + keywords + tags, link to historical notes, then **memory evolution**: a new note triggers rewrites of linked notes' attributes. Event-driven rather than every-N-turns. The auto-keyword generation is directly what your auto-lorebook proposal needs.

## A9. Cadence reference for §6's cost model

SillyTavern's **Objective** extension is the cheapest working callback mechanism in the wild: a plain decrementing counter on `MESSAGE_RECEIVED` (skipping `continue`/`quiet`/`impersonate`), `checkFrequency: 3` → **~0.33 extra calls per message**. Completion detection is a bare `taskResponse.includes('true')` — no structured output, no confidence. Crude, but it's the floor: my §6 director pass is ~3× that cost and should justify the difference.

---

## Revised bottom line

Nothing in §1–§4 or §7's layout changes. The concrete edits:

1. **§5 schema** — add `valid_from`/`valid_until`/`superseded_by` to facts (A1); add certainty tags to `known_by` (A6); consider PHASE-Tree's identity/persona/session/moment layering instead of one flat sheet (A8).
2. **§5 update path** — route by task locality: patch for 1–2 field turns, bucket rewrite on scene restructure (A5). Adopt Talemate's `turns:10 / focus_lines:3 / eviction_threshold:3 / max_items:10` and its single-flight guard (A7). Validate patches pre-commit against schema + write contracts + invariants (A5).
3. **§5 UI** — clear-then-recreate for targeted field regeneration (A3).
4. **§7** — freeze the recall/injection set, recompute only on message-count / summary-count / context-% triggers (A4).
5. **§8** — replace the crude clamp with BST's confidence-scaled clamp plus stickiness on categorical fields; seed new characters from defaults with no model call (A2).
6. **§3** — CHARM is now the headline citation: hallucination is a compliance failure, not an awareness failure. Keep the validator. If it proves too costly, Grounded Continuation's classify-then-check-in-code is the cheap substitute (A6).

**One benchmark to steal as a regression harness:** [Scale-Conditioned Evaluation of Agent Memory](https://arxiv.org/abs/2605.07313) — hold the evidence fixed, add *irrelevant* sessions, measure where accuracy breaks. HippoRAG loses 16–20 points purely from irrelevant-session accumulation. For a month-long story with thousands of messages, that is the exact failure shape you need a test for.